...
const MAX_PANEL_LOGS = Math.max(100, parseInt(process.env.MAX_PANEL_LOGS || "300", 10) || 300);
const panelLogs = [];
...
function addPanelLog(level, args = []) {
  try {
    panelLogs.push({
      ts: new Date().toISOString(),
      level,
      text: args.map(v => v instanceof Error ? (v.stack || v.message || String(v)) : (typeof v === "string" ? v : JSON.stringify(v))).join(" "),
    });
    if (panelLogs.length > MAX_PANEL_LOGS) panelLogs.splice(0, panelLogs.length - MAX_PANEL_LOGS);
  } catch {}
}
for (const level of ["log", "warn", "error"]) {
  console[level] = (...args) => {
    addPanelLog(level, args);
...
app.get("/logs-api", (_req, res) => res.json({ status: "ok", count: panelLogs.length, logs: panelLogs.slice(-200) }));
app.get("/logs", (_req, res) => res.send(`<!DOCTYPE html><html><head><title>Mias MDX Panel Logs</title><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{margin:0;background:#0b1110;color:#d9fbe8;font-family:ui-monospace,monospace;padding:20px}.wrap{max-width:1100px;margin:0 auto}pre{background:#07100d;border:1px solid #1c332b;border-radius:14px;padding:14px;white-space:pre-wrap;word-break:break-word;max-height:80vh;overflow:auto}a,button{background:#25D366;color:#052010;border:none;border-radius:10px;padding:10px 14px;font-weight:700;cursor:pointer;text-decoration:none;margin-right:8px}</style></head><body><div class="wrap"><h1>🧪 Mias MDX Panel Logs</h1><p>Live logs for pairing / QR / validation routes.</p><p><a href="/">Back</a><button onclick="refreshNow()">Refresh</button></p><pre id="logs">Loading...</pre></div><script>async function refreshNow(){const r=await fetch('/logs-api',{cache:'no-store'});const d=await r.json();document.getElementById('logs').textContent=(d.logs||[]).map(function(l){return '[' + l.ts + '] ' + String(l.level||'log').toUpperCase() + ' ' + (l.text||'');}).join('\n')||'No logs yet';}refreshNow();setInterval(refreshNow,5000);</script></body></html>`));
